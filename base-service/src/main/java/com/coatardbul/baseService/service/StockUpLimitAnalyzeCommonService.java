...
public class StockUpLimitAnalyzeCommonService {
    public Map convertFirstUpLimit(JSONObject jsonObject, String dateStr) {
        ...
        for (String key : keys) {
            ...
            //昨日竞价涨幅
            if (key.indexOf("收盘价") > -1 && key.indexOf(dateStr) == -1 && key.indexOf("{-}") < 0
            ) {
            ...
            if (key.indexOf("收盘获利") > -1 && key.indexOf(dateStr) > -1
            ) {
                jsonMap.put("earnProfit", jsonObject.get(key));
            }
            if (key.indexOf("集中度90") > -1 && key.indexOf(dateStr) > -1
            ) {
                jsonMap.put("concentrationRatio", jsonObject.get(key));
            }
            if (key.indexOf("平均成本") > -1
            ) {
                jsonMap.put("jettonCost", jsonObject.get(key));
            }
            if (key.indexOf("所属同花顺行业") > -1
            ) {
                jsonMap.put("thsIndustry", jsonObject.get(key));
            }
            if (key.indexOf("所属概念") > -1
            ) {
                jsonMap.put("theirConcept", jsonObject.get(key));
            }
...
//        }
    ...
    public Map convert(JSONObject jsonObject, String dateStr) {
        ...
        for (String key : keys) {
            ...
            if (key.indexOf("最低价") > -1 && key.indexOf(dateStr) > -1 && key.indexOf("{/}") < 0
            ) {
                jsonMap.put("minPrice", jsonObject.get(key));
            }
            if (key.indexOf("总市值") > -1 && key.indexOf(dateStr) > -1
            ) {
                jsonMap.put("marketValue", jsonObject.get(key));
            ...
            if (key.indexOf("涨跌幅") > -1 && key.indexOf(dateStr) < 0 && key.indexOf("最新") < 0
            ) {
                jsonMap.put("lastIncreaseRate", jsonObject.get(key));
            }
            if (key.indexOf("成交额") > -1 && key.indexOf(dateStr) > -1 && key.indexOf("{/}") < 0
            ) {
                jsonMap.put("tradeAmount", jsonObject.get(key));

...
public class BuyTask extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext context) {
        ...
        String userName=stockTradeBuyTask.getTradeUserId();
        Boolean flag = false;
        ...
        if ("TIME_BUY".equals(stockTradeBuyTask.getStrategySign())) {
            flag = stockTradeService.directBuy(convertAmount(stockTradeBuyTask.getTradeAmount()), convertAmount(stockTradeBuyTask.getTradeNum()), stockTradeBuyTask.getStockCode(), stockTradeBuyTask.getStockName(), userName);
        }


        // V 起来买，
        if ("V_BUY".equals(stockTradeBuyTask.getStrategySign())) {
            flag = vBuyTradeService.tradeProcess(convertAmount(stockTradeBuyTask.getTradeAmount()), convertAmount(stockTradeBuyTask.getTradeNum()), stockTradeBuyTask.getStockCode(),userName);
        }

        //  涨停买入 还需优化，不太智能
        if ("UPLIMIT_BUY".equals(stockTradeBuyTask.getStrategySign())) {
            flag = uplimitBuyTradeService.tradeProcess(convertAmount(stockTradeBuyTask.getTradeAmount()), convertAmount(stockTradeBuyTask.getTradeNum()), stockTradeBuyTask.getStockCode(),userName);
        }
        //  涨停前五档买入
        if ("UPLIMIT_BUY_FIVE".equals(stockTradeBuyTask.getStrategySign())) {
            flag = uplimitFiveBuyTradeService.tradeProcess(convertAmount(stockTradeBuyTask.getTradeAmount()), convertAmount(stockTradeBuyTask.getTradeNum()), stockTradeBuyTask.getStockCode(),userName);
        }
        //涨幅大于多少买入
        if ("RATE_GREATE_BUY".equals(stockTradeBuyTask.getStrategySign())) {
            ...
            flag = rateGreateBuyTradeService.tradeProcess(greateRate, convertAmount(stockTradeBuyTask.getTradeAmount()), convertAmount(stockTradeBuyTask.getTradeNum()), stockTradeBuyTask.getStockCode(),userName);
        }
        ...
        if ("REBOUND_GREATE_BUY".equals(stockTradeBuyTask.getStrategySign())) {
            ...
            flag = reboundGreateBuyTradeService.tradeProcess(reboundRate, convertAmount(stockTradeBuyTask.getTradeAmount()), convertAmount(stockTradeBuyTask.getTradeNum()), stockTradeBuyTask.getStockCode(),userName);
        }

        //T板前五回封
        if ("UPLIMIT_T_FIVE_BUY".equals(stockTradeBuyTask.getStrategySign())) {
            flag = upLimitTfiveBuyTradeService.tradeProcess(convertAmount(stockTradeBuyTask.getTradeAmount()), convertAmount(stockTradeBuyTask.getTradeNum()), stockTradeBuyTask.getStockCode(),userName);
        }

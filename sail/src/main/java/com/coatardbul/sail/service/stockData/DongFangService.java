...
@Service
@Slf4j
public class DongFangService extends CommonService implements DataServiceBridge {
     ...
     * @param map
     */
    private void rebuildStockDetailMap(String response, Map map) {
...
//            map.put("minPrice", split1[7]);
            ...
            //流通市值
            map.put("marketValue", data.get("f117"));
            //暂时不处理
//            map.put("tradeBuySell5Time", split1[31]);
    ...
    private  List<TickInfo>  getStockTickDetail(String code, String response) {
        int beginIndex = response.indexOf("(");
        int endIndex = response.lastIndexOf(")");
        response = response.substring(beginIndex + 1, endIndex);
        JSONObject jsonObject = JSONObject.parseObject(response);
